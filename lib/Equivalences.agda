...
module _ {ℓ₁ ℓ₂ : Level} {X : Type ℓ₁} {Y : Type ℓ₂} where
  ...
  hae-τ-to-ν f g η ε τ y = nat-η-ηg ◾ (!ηgfg [1,1,2] (gτg ◾ ε-fg-comm) [2,1,1] ηg) ◾ nat-η-gε
    where gτg = ap∘ _ _ _ ◾ ap (ap g) (τ (g y))
          ε-fg-comm = ap (ap g) (htpy-id-comm (f ∘ g) ε y) ◾ ! (ap∘ _ _ _) 
          !ηgfg = ! (η (g (f (g y))))
          ηg = η (g y)
          nat-η-ηg = ! (ap-id _) ◾ l₂=!l₁◾r (nat η ηg)
          nat-η-gε = (! (η (g (f (g y)))) [1,1,2] (ap∘ (g ∘ f) g (ε y)) [2,1,1] ηg)
  ...
  module _ (f : X → Y) (e : is-qinv f) where
    ...
    qinv-is-hae-τ x = nat-ε-fη
                      ◾ ! (ε (f (g (f x)))) [1,1,2] η-gf-comm [2,1,1] ε (f x)
...
module _ {ℓ₁ ℓ₂ ℓ₃ : Level} {X : Type ℓ₁} {Y : Type ℓ₂} {Z : Type ℓ₃} where
  ...
  (f , g , η , ε , τ) ● (f' , g' , η' , ε' , τ') = (f● , g● , η● , ε● , τ●)
    where f● = f ∘ f'
          ...
          τ● : ap f● ∘ η● ∼ ε● ∘ f●
          τ● x = ap◾ f● _ _
                 ◾ (! (ap∘ _ _ _) [2,1,2] ap∘ _ _ _ ◾ ap (ap f) (τ' x))
                 ◾ ! (nat (ap f ∘ ε') (η (f' x)))
                 ◾ (ap f (ε' (g (f (f' x)))) [1,1,2] τ (f' x))
